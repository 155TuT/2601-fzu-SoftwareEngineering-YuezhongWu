...
所有断点的外壳内边距为 `4px 4px 4px 16px`，配合 1px 边框和 44px 高控件，总高 54px。两列网格中标题占左列、搜索占右列；桌面搜索框和手机圆形按钮距顶栏右内边缘均为 4px。标题区域距左内边缘为 16px，最大宽 240px，使用 16/18px 字号；签名向右缩进一个汉字，过长时省略。

`mountHeader()` 在页脚存在后将原统计节点移入 `#footer`，所有尺寸一致。已移除 `compactHeader`、1099px 的统计移动监听及宽屏移回分支；搜索创建也不再依赖统计节点。发现阶段等待页脚出现，CSS 在解析过程中隐藏尚位于导航内的统计，避免顶栏临时多出一列。保留原节点和内部 ID，供平台异步更新数字；页脚显式清除 Custom 的 float:right，在正常文档流中显示。
...
桌面 `--blog-sidebar-width:220px` 比 Custom 原有的 230px 内容宽度减少 10px，资料卡跟随占满；原模板左侧 5px padding 保留，因此侧栏外部宽度为 225px。主栏 `.forFlow` 的左预留为 `calc(var(--blog-sidebar-width) + 34px)`，即 254px，资料卡右侧到主内容区仍有 29px。手机隐藏侧栏，主栏继续清除左预留。两枚统计框使用两等列和 4px gap，每框宽 93px。

公告使用 `word-break:normal` 覆盖侧栏继承的 `break-all`，避免将“2601”这样的短数字拆到两行；`overflow-wrap:anywhere` 仍为超长单词或 URL 提供折行兜底。
...
| 视口宽度 | 布局 |
|---|---|
| ≥768px | 标题和靠右搜索框单行，统计在页脚；高 54px，左侧 16px，其余内边距 4px |
| ≤767px | 标题与右侧圆形搜索按钮单行，统计在页脚；高 54px，左侧 16px，其余内边距 4px |
