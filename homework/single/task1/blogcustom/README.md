...
示例页面：[博客首页](https://www.cnblogs.com/155TuT/) · [随笔详情](https://www.cnblogs.com/155TuT/p/22857676)。截至 2026-09-07，当前已部署简化 outline 顶栏：桌面标题、居中搜索、右侧统计；删除六个导航按钮；右下角“更多设置”展开齿轮管理、日/月配色切换，两个图标在 15°/75° 处展开。默认采用浏览器配色；手机顶栏单行、54px 高，右侧为圆形搜索按钮，统计移入页脚。顶栏左右内边距为 16px；文章标题使用 Markdown 一级标题的 28px 字号，正文与摘要图片采用 22px 圆角描边并裁切四角。所有 22px 圆角卡片、粉丝/关注框和内容图片共用右下角两条细斜线与小三角。各轮验证结果及未验证事项见 [验证记录](docs/verification.md)。
...
顶栏所有断点均为单行，高 54px：搜索控件高 44px，上下内边距各 4px，左右各 16px，另计 1px 外壳边框。标题和统计移除描边及角饰；侧栏卡片采用下述公共角饰。

767px 及以下顶栏显示标题和靠右的 44px 圆形搜索按钮。点击后在顶栏下方浮出输入框，自动聚焦；再次点击、Escape 或点外部收起，不改变顶栏高度。页面左右 12px 外边距，侧栏隐藏。768–1099px 为标题和搜索单行。1099px 及以下统计移入页脚，跨越断点时原节点会移回顶栏。横屏至少 1024px 时，内容继续居中占约 70% 宽度。
...
| 顶栏布局 | 各断点高 54px、上下 4px、左右 16px；标题和统计没有独立轮廓 | `page-custom.css` |
| 原生导航 | 移除博客园、首页、新随笔、联系、订阅、管理六个按钮；标题链接返回博客首页 | `scripts/blog-shell.js` |
| 桌面搜索与统计 | ≥1100px 搜索几何居中，统计靠右；≤1099px 统计原节点移入页脚 | CSS、`blog-shell.js` |
